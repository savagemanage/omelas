# Omelasian Bible: Translation Glossary (Korean to English)

This glossary fixes the canonical English rendering of every proper term and name
in the Omelasian Bible. All English translations of the nine books (FEAT-002 through
FEAT-004) must use these renderings exactly and identically, so the 54 chapters stay
consistent for side-by-side reading against the Korean canon.

Rules:

- The Korean prose under `books/` and `_ko/` is the canon and is never altered. The
  English translation lives in the `_en/` collection.
- The English rendering below is authoritative. Do not invent alternate spellings.
- No em dash (U+2014) may appear in any English file or in this glossary. Use straight
  hyphens, commas, or rephrase.
- Preserve Korean place and sensory detail (Gangnam, Gangnam Station Exit 10, the Han
  River, and so on).
...
|--------|---------------------|
| 모든 것은 주님의 영광을 위하여 | All is for the glory of the Lord |

This single rendering is used identically everywhere the refrain appears.

## Names (fixed romanization)

| Korean | English (canonical) | Role |
|--------|---------------------|------|
| 한서린 | Han Seorin | The Untransformed protagonist. Referred to by given name as "Seorin". |
| 이가온 | Lee Gaon | The Gangnam Evangelist. |
| 정도현 | Jung Dohyun | The doubting Engineer. |
| 서명주 | Seo Myungju | The Scientist allied with the Evils. Referred to as "Myungju" where the Korean uses 명주. |
...
These are not top-level canon names but recur within Books 1 through 3; fix them here
so later books stay consistent.

| Korean | English (canonical) | Role |
|--------|---------------------|------|
| 세아 | Sea | David's daughter (Genesis, Infancy Gospel). |
| 지우 | Jiwoo | Seorin's baptized friend (Revelation). |
...
- Keep the dark, contemplative, scripture-like register.
- Keep paragraph breaks aligned with the Korean so side-by-side reading works.
- Translate the full prose. Do not summarize or abridge.
- Korean given-name usage (calling a character by given name) maps to the given name in
  English (Seorin, Myungju), matching the intimacy of the Korean.
